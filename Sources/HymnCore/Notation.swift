...
public enum Notation {
    ...
    static func slices(_ tune: Tune) -> [Slice] {
        var output: [Slice] = []; var tick = 0
        for (i, note) in tune.melody.enumerated() {
            ...
            while left > 0 {
                ...
                let available = min(left, barEnd-tick)
    ...
    public static func payload(_ score: Score, only voice: Voice? = nil) throws -> NotationPayload {
        ...
        guard !parts.isEmpty, parts.allSatisfy({ $0.notes.count == t.melody.count }) else { throw HymnError.invalid("Cannot engrave mismatched parts.") }
        ...
        for measure in 1...max(t.measureCount,1) {
            ...
            for (staff, part) in parts.enumerated() {
                ...
                for slice in current {
                    ...
                    events.append(.init(id: id, noteIndex: slice.index, voice: part.voice, tick: slice.tick, ticks: slice.ticks, pitch: note.pitch, lyric: note.lyrics.first?.text ?? ""))
                }
                mei += "</layer></staff>"
            }
    ...
    public static func musicXML(_ score: Score, only voice: Voice? = nil) throws -> String {
        try score.tune.validated()
        let t = score.tune, slices = slices(t), parts = score.effectiveParts.filter { voice == nil || $0.voice == voice }
